#!/usr/bin/env python3
"""
Custom plotting script for steering and system prompt experiments.
...
SYSTEM_PROMPT_EXPERIMENTS = {
    'system_prompt': 'SysPrompt_Base',
    'system_prompt_no_spec': 'SysPrompt_NoSpec',
    'system_prompt_taboo_words': 'SysPrompt_Taboo',
}

# Combined mapping (3 steering + 3 system prompt = 6 experiments)
EXPERIMENT_MAPPING = {**STEERING_EXPERIMENTS, **SYSTEM_PROMPT_EXPERIMENTS}
    ...
    pareto_configs = [
        {
            ...
            'ylabel': 'Task Completion (%)',
            'title': 'BC Rate wrt Deploy vs Task Completion',
            'x_invert': False,  # Lower is better (less change from deployment)
            'y_invert': True,   # Higher is better
        },
        {
            'x': 'bc_rate_wrt_deploy',
...
def main():
    ...
    output_dir = Path('outputs/6_experiments_plots')
    ...
    print(f"\n{'='*100}")
    print(f"PLOTTING STEERING AND SYSTEM PROMPT EXPERIMENTS")
    print(f"{'='*100}\n")
    ...
    if len(df) != 6:
        print(f"Warning: Expected 6 experiments (3 steering + 3 system prompt), found {len(df)}")
